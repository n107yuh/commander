              ...
              {standings.map((p, i) => {
                ...
                return (
                  <ClickableRow key={p.name} href={`/players/${encodeURIComponent(p.name)}`} className="border-b border-slate-800/50 last:border-0 hover:bg-slate-800/20">
                    ...
                    <td className="text-right px-3 py-3 text-emerald-400 font-mono font-medium">{p.wins}</td>
                    <td className="text-right px-3 py-3 text-red-400 font-mono font-medium">{p.losses}</td>
